node {
    try {
        ...
        stage('Build image') {
            app = docker.build("locanbabu/sunbird-rc-core","target")
            claimApp = docker.build("locanbabu/sunbird-rc-claim-ms","java/claim")
        }
...
//             sh "ssh kesavan@10.4.0.6 'kubectl set image deployment/registry registry=locanbabu/sunbird-rc:${env.BUILD_NUMBER} --record --namespace=ndear'"
//             sh "ssh kesavan@10.4.0.6 'kubectl set image deployment/claim-ms claim-ms=locanbabu/sunbird-rc-claim-ms:${env.BUILD_NUMBER} --record --namespace=ndear'"
//         }
